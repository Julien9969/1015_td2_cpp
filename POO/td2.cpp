...
void ListeFilms::ajoutFilmDansListe(Film* film)
{
	...
	if (capacite_ == 0)
	{
		...
		delete[]elements_;
		elements_ = nouvelleListFilm;
		capacite_ = 1;
	}
	else if (capacite_ <= nElements_)
	{
		nouvelleListFilm = new Film * [capacite_ * 2];
		for (int i : range(nElements_))
		{
			nouvelleListFilm[i] = elements_[i];
		}
...
{
	Film** nouvelleListFilm = new Film * [capacite_];
	...
	for (int i : range(nElements_))
	{
		...
		{
			estRetirer == false ? nouvelleListFilm[i] = elements_[i] : nouvelleListFilm[i - 1] = elements_[i];
		}
	}
...
Film* lireFilm(istream& fichier, ListeFilms& listFilm)
{
	...
	for (int i = 0; i < film->acteurs.nElements; i++) {
		...
		acteur->joueDans.ajoutFilmDansListe(film); //TODO: Ajouter le film à la liste des films dans lesquels l'acteur joue.
	}
