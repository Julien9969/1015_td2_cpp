...
span<shared_ptr<Acteur>> ListeActeurs::spanListeActeurs() const { return span(elements_.get(), nElements_); }
...
{
	//shared_ptr<Film> film = make_shared<Film>(lireString(fichier), lireString(fichier), lireUint16(fichier), lireUint16(fichier), ListeActeurs(lireUint8(fichier)));
	Film* film = new Film;
...
ListeFilms::ListeFilms(const string& nomFichier) : possedeLesFilms_(true)
{
	...
	for ([[maybe_unused]] int i : range(nElements)) { //NOTE: On ne peut pas faire un span simple avec spanListeFilms car la liste est vide et on ajoute des éléments à mesure.
		ajouterFilm(lireFilm(fichier, *this));
		  //NOTE: L'utilisation explicite de this n'est pas dans la matière indiquée pour le TD2.
...
void afficherActeur(const Acteur& acteur, ostream& o)
{
...
*/
template<typename T> 
int ListeFilms::trouverFilmCorrespondant(const T& critere) {
	for (int i : range(nElements)) 
	{
...
int main()
{
	...
	Film skylen = listeFilms[0];
	skylen.titre = "Skylien";
	skylen.acteurs[0] = listeFilms[1].acteurs[0];
	skylen.acteurs[0]->nom = "Daniel Wroughton Craig";
	cout << skylen << ligneDeSeparation << listeFilms[0] <<ligneDeSeparation <<listeFilms[1];
	

	cout << ligneDeSeparation << "Le film trouver est : " << ligneDeSeparation;
	int filmTrouve = listeFilms.trouverFilmCorrespondant([](Film* v) {return v->recette == 955; });
	cout << listeFilms[filmTrouve];
