...
span<shared_ptr<Film>> ListeFilms::enSpan() const { return span(elements_.get(), nElements_); }
...
shared_ptr<Acteur> ListeFilms::trouverActeur(const string& nomActeur) const
{
	for (const shared_ptr<Film> film : enSpan()) {
		for (shared_ptr<Acteur> acteur : film->lireActeur().spanListeActeurs()) {
...
shared_ptr<Film> lireFilm(istream& fichier, ListeFilms& listeFilms)
{
	...
	film = Film (ListeActeurs(lireUint8 (fichier)));  //NOTE: Vous avez le droit d'allouer d'un coup le tableau pour les acteurs, sans faire de réallocation comme pour ListeFilms.  Vous pouvez aussi copier-coller les fonctions d'allocation de ListeFilms ci-dessus dans des nouvelles fonctions et faire un remplacement de Film par Acteur, pour réutiliser cette réallocation.
	...
	//filmp->acteurs.elements = make_unique<Acteur* []()>;
	/*
	//]
